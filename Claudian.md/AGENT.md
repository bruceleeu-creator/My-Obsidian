...
> 本库另有保留文件：`执行2026/`（周计划执行表 md 归档）、`周计划执行表/`（原 HTML 周计划，已归档为 md）——与 ACT 结构共存，不冲突。
...
- [ ] Dataview 渲染曾有问题（Mac 库 2026-08-18 诊断中，Bruce 重启后未回报结果）——若系统已正常显示可忽略
- [ ] git 提交身份是自动生成的 `BruceLeeu <...>`——若需规范可配置 user.name/email
- [ ] Windows 库重启 Obsidian 后确认：日记生成到 `任务/日志/` 且带模板、模板命令面板可用、关系图谱按类型上色、Dataview 代码块正常渲染（2026-08-19 配置完毕待验证）
